...
//-----------------------------
	...
	function addAttributeToProduct($get) {
        ...
        if (AM_USE_ATTRIBUTE_CODE) {
          $this->getAndPrepare('code_suffix', $get, $code_suffix);
          if((empty($code_suffix))||($code_suffix==' ')){
            $code_suffix='';
          }
          $data['code_suffix'] = $code_suffix;
		  
		  if (AM_USE_ATTRIBUTE_CODE_SORT) {
			$this->getAndPrepare('suffix_sort_order', $get, $suffix_sort_order);
		    if((empty($suffix_sort_order))||($suffix_sort_order==' ')){
                $suffix_sort_order='';
            }
			$data['suffix_sort_order'] = $suffix_sort_order;
		  } 
        }
...
// Begin QT Pro Plugin	
    ...
    function repairStock() {
        $query = amDB::query("select sum(products_stock_quantity) as summa from products_stock where products_id='" . $this->intPID . "' and products_stock_quantity>0");
	...
	function removeStockOptionValueFromProduct($get) {
		$this->getAndPrepare('option_id',$get,$optionId);
		amDB::query("delete from products_stock where products_stock_id = '$optionId'");// and products_id = '$this->intPID'");
	...
	function addStockToProduct($get) {
    	...
    	if (($inputok)) {
            ...
            if (!empty($val_array)) {
    		  ...
    		  $q = amDB::query("select products_stock_id as stock_id from products_stock where products_id ='$this->intPID' and products_stock_attributes='" . $val . "' order by products_stock_attributes");
    		  if (amDB::numRows($q) > 0) {
    			  ...
    			  if ($stockQuantity=intval($stockQuantity)) {
    				  ...
    				  amDB::perform('products_stock', $data, 'update', "products_stock_id=$stock_id");
    			  } else {
        		  ...
        		  amDB::perform('products_stock', $data);
        	  }
	...
	function updateProductStockQuantity($get) {
		...
		amDB::perform('products_stock',$data, 'update',"products_stock_id='$products_stock_id'");
...
//-----------------------------
			...
			//print_R($res);
				...
				// Product Attribut Code
                if (AM_USE_ATTRIBUTE_CODE) {
                  $this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['options_values_id']]['code_suffix'] = $res['code_suffix'];
				  if (AM_USE_ATTRIBUTE_CODE_SORT) {
					$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['options_values_id']]['suffix_sort_order'] = $res['suffix_sort_order'];
				  }
                }
//----------------------------
// Change: Add download attributes function for AM
