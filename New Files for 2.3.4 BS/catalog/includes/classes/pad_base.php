    ...
    function _draw_stocked_attributes() {
      ...
      if (sizeof($attributes)>0) {
        foreach ($attributes as $stocked) {
          ...
          $out .=   tep_draw_pull_down_menu('id[' . $stocked['oid'] . ']', array_values($stocked['ovals'])) . "\n";
          $out .= '</div>' . "\n";
        }
      }
      return $out;
...
*/
    function _build_attributes_combinations($attributes, $showoos, $markoos, &$combinations, &$selected_combination, $oidindex=0, $comb=array(), $id="", $text='', $isselected=true, $price='') {
